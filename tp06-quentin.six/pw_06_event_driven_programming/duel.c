...
gboolean on_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    cairo_fill(cr);
    //deuxieme joueur
    cairo_rectangle(cr,x_max,game->p2.rect.y,
...
void set_stop(Game *game)
{
    ...
    char score_format[]="%d";
    char score[2];
    g_snprintf(score,sizeof(score),score_format,game->p1.score);
    gtk_label_set_label(game->p1.label,score);
    gtk_label_set_label(game->p2.label,score);
...
gboolean on_move_disc(gpointer user_data)
{
    ...
    // Works out the ne*w position of the disc.
    
    if(gdk_rectangle_intersect(&game->disc.rect,&game->p2.rect,NULL)==TRUE 
        ||gdk_rectangle_intersect(&game->disc.rect,&game->p1.rect,NULL)==TRUE 
        ||old.x==x_max||old.x==0)
    {
        game->disc.step.x=-game->disc.step.x;
    }
    if(old.y==0||old.y==y_max)
    {
    ...
    // Redraws the disc.
    if(old.x==x_max||old.x==0)
    {
        
        set_pause(game);
        if( game->disc.rect.x==x_max-1)
        {
            game->p1.score+=1;
            // p1_score_label
            char score_format1[]="%d";
            char score1[2];
            g_snprintf(score1,sizeof(score1),score_format1,game->p1.score);
            gtk_label_set_label(game->p1.label,score1);
            if(game->p1.score==END_GAME_SCORE)
            {
                
                GtkWidget* message_fin = gtk_message_dialog_new_with_markup(
                game->ui.window, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
                "Player 1: %d point(s)\nPlayer 2: %d point(s)\n\n<b>The winner is player 1.</b>",game->p1.score,game->p2.score);
                gtk_window_set_title(GTK_WINDOW(message_fin), "Game Over");
                gtk_dialog_run(GTK_DIALOG(message_fin));
                gtk_widget_destroy(message_fin);
                game->p1.score=0;
                g_snprintf(score1,sizeof(score1),score_format1,game->p1.score);
                gtk_label_set_label(game->p1.label,score1);
                set_stop(game);
        ...
        {
            game->p2.score+=1;
            char score_format2[]="%d";
            char score2[2];
            g_snprintf(score2,sizeof(score2),score_format2,game->p2.score);
            gtk_label_set_label(game->p2.label,score2);
...
// Timeout function called at regular intervals to move the player.1 upwards
...
gboolean on_change_window(GtkWidget *widget,GdkEvent *event, gpointer user_data)
{
    ...
    gint y_d_max=gtk_widget_get_allocated_width(GTK_WIDGET(game->ui.area))-game->disc.rect.height;
    gint x_d_max=gtk_widget_get_allocated_width(GTK_WIDGET(game->ui.area))-game->disc.rect.width;

    GdkRectangle old = game->p2.rect;
    GdkRectangle old_d=game->disc.rect;

    game->p2.rect.x = CLAMP(game->p2.rect.x+game->p2.rect.width, 0, x_max);
    game->disc.rect.x=CLAMP(game->disc.rect.x,0,x_d_max);
    game->disc.rect.y=CLAMP(game->disc.rect.y,0,y_d_max);

    redraw_item(game->ui.area,&old_d,&game->disc.rect);
    redraw_item(game->ui.area, &old, &game->p2.rect);
...
int main (int argc, char *argv[])
{
    ...
    g_signal_connect(area,"configure_event",G_CALLBACK(on_change_window),&game);
